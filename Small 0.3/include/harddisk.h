...
#define ECC_STAT	(1 << 2)		// 2: ECC校验错。当遇到一个可恢复的数据错误而且以得到纠正，就会置该位。这种情况不会中断一个多扇区读操作。
...
// 命令寄存器(0x1f7)命令列表:
// NOTE! 以下是默认的命令，其中后面加上0x0的是固定命令，其他是可变命令，其中可变的二进制位参见各个命令后面的说明。
...
extern void insert_hd_request(struct buffer_head * bh, unsigned int cmd, unsigned int sectors, unsigned int nr_v_sector);
