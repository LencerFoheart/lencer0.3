...
void print_trap_msg(unsigned long cr2, unsigned long cs, unsigned long eip, unsigned long error_code)
{
	int i = 0;
	for(; i<NR_PROC; i++)
	{
	...
	printf("[process-nr]%u [pid]%u [RING]%u [EIP]%x [cr2]%x [errorcode]%x.\n",
		i, current->pid, (cs & 0x3), eip, cr2, error_code);	
}

/*
 * 在本版内核中，为了调试的方便，对这些中断做简明处理。只显示一些信息，然后panic.
 * 其中do_no_page()函数在mm中。
 */
...
{
	printf("Kernel MSG: do_others: other-IRQ-interrupt.");
}

void trap_init(void)
{
	set_idt(INT_R0, page_fault_int, NR_PF_INT);
	set_idt(INT_R0, double_fault_int, NR_DF_INT);
	set_idt(INT_R0, error_tss_int, NR_TS_INT);
	set_idt(INT_R0, no_segment_int, NR_NP_INT);
	set_idt(INT_R0, error_ss_int, NR_SS_INT);
	set_idt(INT_R0, general_protect_int, NR_GP_INT);
	set_idt(INT_R0, align_check_int, NR_AC_INT);
	set_idt(INT_R0, divide_int, NR_DIV_INT);
	set_idt(INT_R0, debug_int, NR_DEBUG_INT);
	set_idt(INT_R0, nmi_int, NR_NMI_INT);
	set_idt(INT_R0, break_point_int, NR_BP_INT);
	set_idt(INT_R0, overflow_int, NR_OF_INT);
	set_idt(INT_R0, bound_int, NR_BR_INT);
	set_idt(INT_R0, ud_int, NR_UD_INT);
	set_idt(INT_R0, nm_int, NR_NM_INT);
	set_idt(INT_R0, mf_int, NR_MF_INT);
	set_idt(INT_R0, machine_check_int, NR_MC_INT);
	set_idt(INT_R0, xf_int, NR_XF_INT);
	...
	/*
	 * Well, I'm very surprising! 在我初始化一段比较大的内存为0时，竟引发了IRQ7中断。
	 * 不知道原因，于是百度之，话说是有这样一种可能性：“当中断请求持续时间太短，当CPU
	 * 反过头来让8259A反馈时，没有了中断请求，于是8259A就会把IRQ7填入data bus”。
	 *
