...
long ticks = 0;									// 嘀嗒数
struct proc_struct * proc[NR_PROC] = {0};		// 进程槽
struct proc_struct * current = NULL;			// 当前进程结构指针

union proc_union init_proc = {INIT_PROC, };		// UNION(初始化进程结构, 进程内核态堆栈)

/*
 * 切换进程。
 *
 * 此处不判断是否 current==proc[next]，因为在调度程序里已进行了判断。
 */
 ...
 * 内核调度程序。
 *
 * NOTE! 进程0的调度不依赖于state字段和counter字段。若除进程0没有时间片大于0的可执行进程，
 * 则重新设置除进程0外的所有进程的时间片，由于可能包含不可执行进程，且counter可能小于0(因为
 * 内核默认在发生时钟中断时，处于内核态的进程不进行调度)，故counter = priority + counter。
 */
BOOL schedule(void)
{
	...
	while(1)
	{
		// 倒序寻找优先级最高的可执行进程。NOTE! 以下不对进程0进行处理
		for(int i=NR_PROC-1; i; i--)
		{
			if((NULL != proc[i]) && (RUNNING == proc[i]->state) && (counter < proc[i]->counter))
			{
				next = i;
 ...
 * 对于可中断睡眠，由于收到信号并转为可执行状态的进程不一定是睡眠进程队列的头部进程，故需唤醒
 * 可中断睡眠进程队列的头部进程，以便在其中任何一个收到信号后，队列中所有的进程均转为可执行态。
 */
...
void sched_init(void)
{
	...
	// 设置进程0的ldt和tss描述符
	set_ldt_desc(0, &(proc[0]->ldt));
	set_tss_desc(0, &(proc[0]->tss));
	// 装载进程0的ldt和tss选择符，只明确加载一次，后续任务切换时由CPU自动加载
	__asm__("lldt	%%ax" ::"a"(_LDT(0)) :);
	__asm__("ltr	%%ax" ::"a"(_TSS(0)) :);
	// 挂载时钟中断处理程序
